...
public static class PgVectorExample
{
   ...
   public static async Task RunAsync()
   {
      ...
      var results = await ragProvider.SearchAsync(collectionName, "What are microservices disadvantages?", 3);
      foreach (var r in results) Console.WriteLine($"  [{r.Score:F3}] {r.Text[..Math.Min(120, r.Text.Length)]}...");

      // ── 5. Get full context ──
      Console.WriteLine("\n📚 Context retrieval:\n");
      var context = await ragProvider.GetContextAsync(collectionName, "microservices vs monolith tradeoffs", 3);
