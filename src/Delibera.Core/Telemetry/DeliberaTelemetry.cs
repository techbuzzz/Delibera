...
public static class DeliberaTelemetry
{
        ...
        int maxRounds)
    {
        return DeliberaActivitySource.StartActivity(
            ...
            [
                new(DeliberaTelemetryTags.DebateId, debateId),
                new(DeliberaTelemetryTags.StrategyName, strategyName),
                new("debate.member_count", memberCount),
                new("debate.max_rounds", maxRounds),
            ]);
    }
    ...
    public static Activity? StartRound(int roundNumber, string roundName)
    {
        return DeliberaActivitySource.StartActivity(
            ...
            [
                new(DeliberaTelemetryTags.RoundNumber, roundNumber),
                new(DeliberaTelemetryTags.RoundName, roundName),
            ]);
    }
    ...
    public static Activity? StartMemberRespond(string memberName, string? memberRole)
    {
        ...
        if (!string.IsNullOrEmpty(memberRole))
            tags.Add(new(DeliberaTelemetryTags.MemberRole, memberRole));
    ...
    {
        var truncated = query.Length > 200 ? query[..200] : query;
        return DeliberaActivitySource.StartActivity(
    ...
    {
        var truncated = task.Length > 200 ? task[..200] : task;
        return DeliberaActivitySource.StartActivity(
            DeliberaActivityNames.OperatorExecute,
            [
                new(DeliberaTelemetryTags.RequesterName, requesterName),
                new("operator.task", truncated),
            ]);
    }
    ...
    public static void RecordDebateCompleted(double durationMs, string strategyName, bool success)
    {
        ...
        DeliberaMeter.DebatesCompleted.Add(
            ...
            new KeyValuePair<string, object?>(DeliberaTelemetryTags.Success, success ? "true" : "false"));
    }
