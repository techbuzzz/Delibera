...
///    <para>
///       Each collection maps to a dedicated table with schema:
...
public sealed class PgVectorStore : IVectorStore
{
   ...
   ///    <c>"Host=localhost;Database=council;Username=postgres;Password=secret"</c>).
   /// </param>
   ...
   public async Task UpsertAsync(string collectionName, IReadOnlyList<VectorPoint> points, CancellationToken ct = default)
   {
      ...
      foreach (var p in points)
      {
         ...
         cmd.Parameters.AddWithValue(metadataJson is not null
            ? metadataJson
            : DBNull.Value);
