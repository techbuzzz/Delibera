...
///    </para>
///    <list type="bullet">
///       <item>Stores each member's final response (and any final-verdict text) as
///       a memory after the debate completes, tagged with the debate id and member
///       name.</item>
///       <item>Injects the top-K recalled memories into each member's system prompt
///       (clearly marked as <c>[Memory from previous sessions]</c>) at the start of
///       the next debate the member participates in.</item>
///    </list>
///    <para>
...
{
    private readonly System.Collections.Concurrent.ConcurrentDictionary<string, List<MemoryEntry>> _memories = new();
...
{
    private readonly Interfaces.IRagProvider _ragProvider;
    private readonly Interfaces.IEmbeddingProvider _embeddingProvider;
    ...
    public QdrantAgentMemory(
        Interfaces.IRagProvider ragProvider,
        Interfaces.IEmbeddingProvider embeddingProvider,
