...
/// </summary>
/// <remarks>
/// <para>v3.0: Enhanced with per-round Knowledge Keeper queries.
/// The Chairman auto-queries the Knowledge Keeper before each round to provide
/// structured, fact-based context with sources.</para>
/// </remarks>
public sealed class StandardDebate : DebateScenario
{
       ...
       CancellationToken ct = default)
   {
      ...
      if (maxRounds >= 4 && chairman is not null)
      {
         ...
         catch (Exception ex) { finalVerdict = $"[CHAIRMAN ERROR: {ex.Message}]"; }
      }

      return BuildResult(finalVerdict);

      // ── local helper ──
      DebateResult BuildResult(string? verdict = null) => new()
      {
