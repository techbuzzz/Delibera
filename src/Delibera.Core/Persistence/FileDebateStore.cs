...
public sealed class FileDebateStore : IDebateStore
{
    ...
    /// <param name="retentionDays">Optional retention period in days. When set, older checkpoints are deleted lazily on <see cref="ListAsync"/>.</param>
    ...
    public async Task<string> SaveCheckpointAsync(DebateCheckpoint checkpoint, CancellationToken ct = default)
    {
        ...
        var id = string.IsNullOrEmpty(checkpoint.DebateId) ? DebateCheckpoint.GenerateId() : checkpoint.DebateId;
        var stamped = checkpoint with { DebateId = id, CreatedAt = checkpoint.CreatedAt == default ? DateTimeOffset.UtcNow : checkpoint.CreatedAt };
            ...
            try
            {
                ...
                if (checkpoint is not null)
                    metas.Add(new DebateCheckpointMeta(
                        ...
                        LastCompletedRound: checkpoint.LastCompletedRound,
                        OriginalQuestion: TruncateForList(checkpoint.OriginalQuestion)));
            }
    ...
    private string GetFilePath(string id) => Path.Combine(_directory, $"{id}.checkpoint.json");

    private static string TruncateForList(string s) =>
        string.IsNullOrEmpty(s) ? string.Empty : s.Length <= 80 ? s : s[..80] + "…";
}