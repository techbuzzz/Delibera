...
public sealed class MarkdownKnowledgeBase : IKnowledgeBase
{
   ...
   public async Task LoadAsync(string source, CancellationToken ct = default)
   {
      ...
      Store(Path.GetFileName(fullPath), content, null);
   }
   ...
   ///    <paramref name="content" /> or <paramref name="sourceName" /> is null or
   ///    whitespace.
   /// </exception>
   /// <exception cref="OperationCanceledException">The token has been canceled.</exception>
      ...
      CancellationToken ct = default)
   {
      ...
      await LoadCoreAsync(content, sourceName, null, ct).ConfigureAwait(false);
   }
   ...
   /// <exception cref="ArgumentException">
   ///    <see cref="KnowledgeDocument.Name" /> or <see cref="KnowledgeDocument.Content" />
