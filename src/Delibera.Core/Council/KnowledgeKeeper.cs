...
public sealed class KnowledgeKeeper
{
      ...
      CancellationToken ct = default)
   {
      ...
      var searchResults = await _ragProvider.SearchAsync(CollectionName, query, limit, ct: ct);
      ...
      var contextText = await _ragProvider.GetContextAsync(CollectionName, query, limit, ct);
      string answer;
