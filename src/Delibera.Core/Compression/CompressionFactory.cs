...
public static class CompressionFactory
{
   ...
   /// <param name="llmProvider">LLM provider required for summarization strategies. May be <c>null</c> for non-LLM strategies.</param>
   /// <param name="modelName">Model name for LLM-based compression. May be <c>null</c> for non-LLM strategies.</param>
   /// <param name="embeddingProvider">Embedding provider for semantic strategies. May be <c>null</c> for non-embedding strategies.</param>
       ...
       IEmbeddingProvider? embeddingProvider = null)
   {
      ...
      {
         CompressionStrategy.None => new PassThroughCompressor(),
   ...
   public static IContextCompressor Create(
       ...
       IEmbeddingProvider? embeddingProvider = null)
   {
      if (!Enum.TryParse<CompressionStrategy>(strategyName, ignoreCase: true, out var strategy))
